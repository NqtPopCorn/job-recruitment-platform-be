...
public class CompanyUploadController {
    ...
    public ResponseEntity<ApiDataResponse<List<UploadDataResponse>>> getImagesOfCompanyById(@PathVariable("companyId") Long id) {
        List<UploadDataResponse> images = companyUploadService.getAllUploadedImage(id);

        ApiDataResponse<List<UploadDataResponse>> response = ApiDataResponse.<List<UploadDataResponse>>builder()
                .statusCode(HttpStatus.OK.value())
    ...
    @PostMapping(value = "/{companyId}/gallery", consumes = MediaType.MULTIPART_FORM_DATA_VALUE)
