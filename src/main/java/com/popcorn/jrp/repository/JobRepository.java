...
public interface JobRepository extends JpaRepository<JobEntity, Long>, JpaSpecificationExecutor<JobEntity> {
    ...
    @Query("SELECT j.skills FROM JobEntity j WHERE j.skills IS NOT NULL AND j.isDeleted = false")
    ...
    @Query("SELECT j.salary FROM JobEntity j WHERE j.salary IS NOT NULL AND j.isDeleted = false")
    List<String> findAllSalariesJson();

    // 10. GET List Job Dashboard
    List<JobEntity> findByEmployerIdAndIsDeletedFalse(Long companyId);
